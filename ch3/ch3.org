...
\noindent Similarly, we can use =multiplier= to define =c/= by noticing that if \( x / y = z\) then \( x = zy \) whenever \( y \neq 0 \)
...
\noindent Do note however, that setting the denominator to =x= to 0 is undefined. Doing so in the current implementation will not give us a nice error message.
