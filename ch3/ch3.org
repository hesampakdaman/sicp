...
*** Exercise 3.34
#+begin_quote
Louis Reasoner wants to build a =squarer=, a constraint device with two terminals such that the value of connector =b= on the second terminal will always be the square of the value =a= on the ﬁrst terminal. He proposes the following simple device made from a =multiplier=:
...
#+end_quote
Suppose connector =a= has no value and we set connector =b= to 25. Since the relation is a square, we expect the system to
give us the value of 5 for connector =a=. However, this does not happen. To understand this let's take a look at the =cond= of =mulitiplier=.
#+begin_src scheme
...
#+end_src
\noindent Here =product= has a value but none of =m1= nor =m2= have. So no case in =cond= applies and =process-new-value= simply does nothing.

\newpage

*** Exercise 3.35
#+begin_quote
Ben Bitdiddle tells Louis that one way to avoid the trouble in Exercise 3.34 is to deﬁne a squarer as a new primitive constraint. Fill in the missing portions in Ben’s outline for a procedure to implement such a constraint:
...
\noindent In case we have a value for connector =b=, then we need to set the value of connector =a= to the square root of =b=. On the other hand, if there's a value for =a= then we set the value of =b= to the square of =a=. The rest of the procedure is very similar to the other primitives we have already seen like =adder=.
#+begin_src scheme
  (define (squarer a b)
    (define (process-new-value)
      (if (has-value? b)
...
*** Exercise 3.37
#+begin_quote
Exercise 3.37: The =celsius-fahrenheit-converter= procedure is cumbersome when compared with a more expression-oriented style of deﬁnition, such as
...
\noindent Here =c+, c*=, etc. are the “constraint” versions of the arithmetic operations. For example, =c+= takes two connectors as arguments and returns a connector that is related to these by an adder constraint:
...
\noindent Deﬁne analogous procedures =c-, c*, c/=, and =cv= (constant value) that enable us to deﬁne compound constraints as in the converter example above.
#+end_quote

\noindent We can define =cv= in terms of =constant= which is already supplied to us.
...
\noindent If we are a little clever we can define =c-= in terms of =adder=. If \( x - y = z\) then we have that \( x = y + z\), therefore
#+begin_src scheme
  (define (c- x y)
    (let ((z (make-connector)))
      (adder y z x)
...
\noindent Multiplication is straight-forward with the use of =multiplier=.
#+begin_src scheme
  (define (c* x y)
    (let ((z (make-connector)))
      (multiplier x y z)
...
\noindent By noticing that if \( x / y = z\) then \( x = zy \) whenever \( y \neq 0 \), we can define =c\= in terms of =multiplier=
...
\noindent Do note however, that setting the denominator =y= to 0 is undefined. Doing so in the current implementation will not give us a nice error message.
...
#+begin_src scheme
  ...
  (probe "ratio" z)
  (set-value! x 1 'user)
  (set-value! y 0 'user)                  ; ERROR: Contradiction (1 0)
#+end_src

\noindent To get better error messages we need to define division as a primitive similar to how =multiplier= is defined in the book. In one of the =conds= we check if =b= is =zero= and error if so. We don't show this implementation here since it is very similar to =multiplier= in the book.
...
#+end_quote
a. If we assume the banking system imposes the transactions to occur in some sequential order, then we have 6 possible combinations to consider:
- \( \$100 \rightarrow \$110 \rightarrow \$90 \rightarrow \$45 \)
- \( \$100 \rightarrow \$110 \rightarrow \$55 \rightarrow \$35 \)
- \( \$100 \rightarrow \$80  \rightarrow \$90 \rightarrow \$45 \)
- \( \$100 \rightarrow \$80  \rightarrow \$40 \rightarrow \$50 \)
- \( \$100 \rightarrow \$50  \rightarrow \$60 \rightarrow \$40 \)
- \( \$100 \rightarrow \$50  \rightarrow \$30 \rightarrow \$40 \)

\noindent b. We draw a diagram to show a possible value that the system could produce if the transactions are allowed to interleave.
[[../images/ch3/ex-3.38/transaction.png]]

\newpage

*** Exercise 3.39
#+begin_quote
Which of the ﬁve possibilities in the parallel execution shown above remain if we instead serialize execution as follows:
#+begin_src scheme
  (define x 10)
...
In =parallel-execute= only =(lambda () (* x x))= and =(set! x (+ x 1))= are serialized. These two cannot be interleaved, they can however be run in any order. Hence, out of the possible five values these three remain:

- 101: \( P_1 \) sets \( x \) to 100 and then \( P_2 \) increments \( x \) to 101.
- 121: \( P_2 \) increments \( x \) to 11 and then \( P_1 \) sets \( x \) to \( x \cdot x \).
- 100: \( P_1 \) calculates \( x \cdot x = 100\), then \( P_2 \) sets \( x \) to 11, then \( P_1 \) sets \( x \) to 100.
...
#+end_src
#+end_quote
\noindent Without using a serializer the possible sequences are:

- 10^6: \( P_1 \) sets \( x \) to 100 and then \( P_2 \) sets \( x \) to \( x^3 \).
- 10^6: \( P_2 \) sets \( x \) to 10^3 and then \( P_1 \) sets \( x \) to \( x \cdot x \).
- 10^4: \( P_1 \) accesses \( x \) (once), then \( P_2 \) sets \( x \) to 10^3, then \( P_1 \) sets \( x \).
- 10^2: \( P_1 \) accesses \( x \) (twice), then \( P_2 \) sets \( x \) to 10^3, then \( P_1 \) sets \( x \).
- 10^5: \( P_2 \) accesses \( x \) (once), then \( P_1 \) sets \( x \) to 10^2, then \( P_2 \) sets \( x \).
- 10^4: \( P_2 \) accesses \( x \) (twice), then \( P_1 \) sets \( x \) to 10^2, then \( P_2 \) sets \( x \).
- 10^3: \( P_2 \) accesses \( x \) (thrice), then \( P_1 \) sets \( x \) to 10^2, then \( P_2 \) sets \( x \).

\noindent Making use of the serializer the two calculations cannot be interleaved leaving only the possible sequence:
...
#+end_quote
The balance of the account will be conserved because both =withdraw= and =deposit= procedures are serialized. However, if the =balance= procedure is not serialized, then reading an account's balance can be non-deterministic. In particular, consider the following set of events
#+begin_src scheme
  (define account (make-account 100))
  (parallel-execute (lambda () ((account 'withdraw) 50))
                    (lambda () (display (account 'balance))))
#+end_src
\noindent Depending on the order of execution of the procedures \(P_{1}\) and \(P_{2}\), the displayed account balance  is either 50 or 100. In this scenario, reading the account's balance can result in anomalous behavior.
