...
#+end_quote
We can use the famous =tortoise-and-hare= algorithm for detecting cycles in a list. The idea is to have two pointers, one that moves one step at a time and another that moves two steps at a time. If there is a cycle in the list, then the two pointers will eventually point to the same =cons= in the list.
